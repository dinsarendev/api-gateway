...
@RestController
@RequestMapping("/api/management/admin/api-keys")
...
public class AdminApiKeyController {
    ...
    @GetMapping
    public Mono<ResponseEntity<List<ApiKey>>> list(ServerWebExchange exchange) {
    ...
    public Mono<ResponseEntity<Map<String, Object>>> create(
            @RequestBody CreateKeyRequest req, ServerWebExchange exchange) {
    ...
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Map<String, Object>>> revoke(@PathVariable Long id, ServerWebExchange exchange) {
        return adminAuth.require(exchange, Permissions.SECURITY_WRITE)
            .then(repository.findById(id)
                .flatMap(k -> {
                    k.setStatus("INACT");
                    k.setUpdatedAt(LocalDateTime.now());
                    k.setUpdatedBy(adminAuth.currentUser(exchange));
